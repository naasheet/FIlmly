...
export const createValidation = [
    ...
    body("expectedRating").optional({ nullable: true }).isFloat({ min: 0, max: 5 }).toFloat(),
    body("expectedNote").optional({ nullable: true }).isString().isLength({ max: 120 }),
    body("actualRating").optional({ nullable: true }).isFloat({ min: 0, max: 5 }).toFloat(),
    body("actualNote").optional({ nullable: true }).isString().isLength({ max: 120 }),
    body("rewatchability").optional({ nullable: true }).isString().isLength({ max: 50 }),
    body("rewatchabilityWhy").optional({ nullable: true }).isString().isLength({ max: 1000 }),
...
export const createHandler: Handler = async (req, res) => {
    ...
    try {
        ...
        const result = await createEntry({
            ...
            mood: req.body.mood,
            expectedRating: req.body.expectedRating,
            expectedNote: req.body.expectedNote,
            actualRating: req.body.actualRating,
            actualNote: req.body.actualNote,
            rewatchability: req.body.rewatchability,
            rewatchabilityWhy: req.body.rewatchabilityWhy,
